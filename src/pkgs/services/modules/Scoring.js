...
export class ForteScoring {
  ...
  async runLatencyTest() {
    ...
    const analyser = this.state.scoring.micAnalyser;
    const pitchDetector = this.state.scoring.pitchDetector;
    ...
    const testPromise = new Promise((resolve, reject) => {
      ...
      const osc = this.audioCore.context.createOscillator();
      const gain = this.audioCore.context.createGain();
      osc.frequency.value = TEST_FREQ_HZ;
      gain.gain.value = 0;

      osc.connect(gain).connect(this.audioCore.masterGain);
      osc.start();

      const baseTime = this.audioCore.context.currentTime + WARMUP_S;
      ...
      setTimeout(() => {
        ...
        const std = Math.sqrt(
          latencies.map((x) => Math.pow(x - mean, 2)).reduce((a, b) => a + b) /
            latencies.length,
        );
        ...
        this.state.scoring.measuredLatencyS = mean;
        resolve(mean);
      }, TIMEOUT_S * 1000);
    });
