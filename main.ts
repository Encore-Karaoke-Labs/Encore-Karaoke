...
export interface DiscoveredUpdateServer {
  id: string;
  ip: string;
  host: string;
  port: number;
...
export interface SongEntry {
  ...
  artist?: string;
  videoPath?: string;
  type?: "multiplexed" | "mid" | "kar" | "midi" | string;
  [key: string]: unknown;
}

export interface SongbookExportPayload {
  folderPath: string;
  libraryTitle: string;
  filePath?: string;
}

export interface SongbookReceivePayload {
  songs: SongEntry[];
  filePath: string;
  libraryTitle: string;
}

export interface SongbookProgressPayload {
  phase?: string;
  current: number;
  total: number;
  percentage: number;
...
export interface KnownLibraryItem {
  path: string;
  manifest: LibraryManifest;
}

export interface RPCSetPayload {
  state: string;
  details?: string;
...
export interface SaveRecordingPayload {
  ...
  micBuffer?: ArrayBuffer | Uint8Array;
  musicBuffer?: ArrayBuffer | Uint8Array;
  songTitle?: string;
}
...
interface Logger {
  info(tag: string, msg: unknown): void;
  warn(tag: string, msg: unknown): void;
  error(tag: string, msg: unknown): void;
  debug(tag: string, msg: unknown): void;
}
...
interface KuroshiroStaticUtil {
  kanaToRomaji(text: string, system: string): Promise<string>;
}
...
const KuroshiroClass = ((
  _Kuroshiro as unknown as { default?: KuroshiroConstructor }
).default ?? _Kuroshiro) as unknown as KuroshiroConstructor & {
  Util: KuroshiroStaticUtil;
};

const KuromojiAnalyzerClass = ((
  _KuromojiAnalyzer as unknown as {
    default?: new () => KuroshiroAnalyzer;
  }
).default ?? _KuromojiAnalyzer) as unknown as new () => KuroshiroAnalyzer;
...
function calculateFileCRC32(filePath: string): Promise<number> {
...
let activeUpdateServers: DiscoveredUpdateServer[] = [];
const serverBrowser = instance.find({ type: "encore-server" });

serverBrowser.on("up", async (service: Service) => {
  try {
    if (!service.referer?.address) return;
    const res = await fetch(
      `http://${service.referer.address}:${service.port}/info`,
    );
    const serverInfo = (await res.json()) as UpdateServerInfo;

    const serverId = `${service.referer.address}:${service.port}`;
    const serverObj: DiscoveredUpdateServer = {
      id: serverId,
...
serverBrowser.on("down", (service: Service) => {
  if (!service.referer?.address) return;
  const serverId = `${service.referer.address}:${service.port}`;
...
instance.find({ type: "encore-server" }, async (service: Service) => {
  try {
    if (!service.referer?.address) return;
    const res = await fetch(
      `http://${service.referer.address}:${service.port}/info`,
...
void kuroshiro.init(new KuromojiAnalyzerClass());
...
try {
  ...
  logger.error("SYSTEM", `Failed to create User BGV directory: ${err.message}`);
}
...
let discordClient = new Client({ clientId: DISCORD_CLIENT_ID });
let rpcReconnAttempts = 0;
let isRpcReconnecting = false;
let currentRPCState = "Booting up...";

function setupDiscordRPC(): void {
  discordClient.on("ready", async () => {
    ...
    try {
      ...
      logger.error("DISCORD", "Failed to subscribe to join events:");
      logger.error("DISCORD", error);
    }

    void discordClient.user?.setActivity({
  ...
  discordClient.on("ACTIVITY_JOIN", (data: unknown) => {
    const secret =
      typeof data === "string"
        ? data
        : (data as { secret?: string } | undefined)?.secret;
  ...
  discordClient.on("ACTIVITY_JOIN_REQUEST", (user: any) => {
    logger.info(
      "DISCORD",
      `Join request received from: ${user?.username ?? "Unknown"}`,
    );
  });
  ...
  discordClient.on("disconnected", () => {
    ...
    const interval = setInterval(() => {
      ...
      discordClient = new Client({ clientId: DISCORD_CLIENT_ID });
      setupDiscordRPC();
      discordClient
        .login()
        .catch(() => logger.error("DISCORD", "Login failed"));
...
server.get("/local_ip", (_req: Request, res: Response) => {
  res.send(local_ip);
});

server.get("/qr", (req: Request, res: Response) => {
...
server.get("/drives", async (req: Request, res: Response) => {
  const fToken = req.query.token;
  if (!fToken) {
    res.status(403).json({ error: "Provide an access token to continue." });
    return;
  }
  if (fToken !== fileToken) {
  ...
  try {
    ...
    const err = error as Error;
    logger.error("FILE", `Failed to get drives: ${err.message}`);
    res.status(500).send(err.message);
  }
});

server.get("/user-bgv-list", async (_req: Request, res: Response) => {
  try {
    if (!fs.existsSync(userBGVDir)) {
      res.json([]);
      return;
    ...
    const err = error as Error;
    logger.error("FILE", `Failed to read User BGVs: ${err.message}`);
    res.status(500).json({ error: err.message });
  }
});

server.post("/list", (req: Request, res: Response) => {
  ...
  if (!dir) {
    res.status(400).json({ error: true, error_msg: "No directory provided" });
    return;
  }

  fs.stat(dir, (err, stats) => {
    if (err || !stats.isDirectory()) {
      res.status(400).json({ error: true, error_msg: "Invalid directory" });
      return;
    }

    fs.readdir(dir, async (readErr, files) => {
      if (readErr) {
        res.status(400).json({ error: true, error_msg: "Read error" });
        return;
      }
...
server.get("/getFile", (req: Request, res: Response) => {
  const fPath = req.query.path as string;
  const fToken = req.query.token;
  if (!fToken) {
    res.status(403).json({ error: "Provide an access token to continue." });
    return;
...
server.get("/yt-search", async (req: Request, res: Response) => {
  const q = req.query.q as string;
  logger.info("YOUTUBE", `Searching: ${q}`);
  const results = (await (youtubesearchapi as any).GetListByKeyword(
    q,
    false,
  )) as unknown;
  res.json(results);
...
server.post("/auth/create-hash", (req: Request, res: Response) => {
...
server.post("/auth/verify-hash", (req: Request, res: Response) => {
  const { password, salt, hash } = req.body as {
    password?: string;
    salt?: string;
    hash?: string;
  };
...
if (zoomFactor === null || zoomFactor === undefined) {
  zoomFactor = (Config.getItem("zoomFactor") as number) ?? 0.85;
}

const getBooleanSetting = (value: unknown): boolean =>
  value === true || value === 1 || value === "true" || value === "1";

const applyZoomFactor = (nextZoomFactor: unknown): number => {
...
const resetZoom = (): number => applyZoomFactor(0.85);
const addZoom = (): number => applyZoomFactor((zoomFactor as number) + 0.15);
const reduceZoom = (): number => {
  if ((zoomFactor as number) > 0.26) {
    return applyZoomFactor((zoomFactor as number) - 0.15);
  }
  return zoomFactor as number;
};
...
const createWindow = (): void => {
  ...
  void appView.webContents.loadURL(`http://127.0.0.1:${PORT}/index.html`);
  appView.webContents.setZoomFactor(zoomFactor as number);
  ...
  appView.webContents.on("devtools-opened", () => {
    ...
    void appView.webContents.devToolsWebContents?.executeJavaScript(
  ...
  const handleSpecialKeys = (event: ElectronEvent, input: Input): void => {
...
void app.whenReady().then(() => {
  ...
  discordClient
    .login()
    .catch(() => logger.error("DISCORD", "Initial login failed"));
  ...
  cloudSocket.on("room-created", (data: { roomCode: string }) => {
  ...
  server.get("/cloud_info", (_req: Request, res: Response) => {
    if (!activeRoomCode) {
      res
        .status(503)
  ...
  ipcMain.on("app-close", (event: IpcMainEvent) => {
    BrowserWindow.fromWebContents(event.sender)?.close();
  ...
  ipcMain.on("simulate-key", (event: IpcMainEvent, keyChar: string) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (!win) return;
    ...
    `,
        ...
        .catch((err: unknown) => console.error("Simulate key error:", err));
    }
  });
  ...
  ipcMain.handle("deep-link-get-initial", () => {
    const url = pendingDeepLinkUrl ?? pendingDeepLinks.shift() ?? null;
    pendingDeepLinkUrl = null;
    return url;
  });
  ipcMain.handle("fullscreen-get", (event: IpcMainInvokeEvent) => {
    const win = BrowserWindow.fromWebContents(event.sender);
  ...
  ipcMain.handle("fullscreen-toggle", (event: IpcMainInvokeEvent) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (!win) return false;
  ...
  ipcMain.on("config-merge", (_event: IpcMainEvent, dataObject: unknown) => {
    if (typeof dataObject !== "object" || dataObject === null) return;
    Config.merge(dataObject as Record<string, unknown>);
  ...
  ipcMain.handle("zoom-set", (_event: IpcMainInvokeEvent, value: unknown) =>
  ...
  ipcMain.handle(
    "romanize",
    async (_event: IpcMainInvokeEvent, rawJapanese: unknown) => {
      ...
      try {
        ...
        romaji = romaji
          ...
          .replace(/\s+([,.\?!♪~～''])/g, "$1") // Remove space right before punctuation/symbols.
        ...
        logger.warn(
          "ROMANIZE",
          `Kuromoji conversion failed for "${rawJapanese}": ${err.message}. Using safe fallback.`,
        );

        try {
          const fallbackRomaji = await KuroshiroClass.Util.kanaToRomaji(
            rawJapanese,
            "hepburn",
          );
          ...
          logger.error(
            "ROMANIZE",
            `Fallback romanization failed: ${fbErr.message}`,
          );
          return rawJapanese;
        }
      }
  ...
  ipcMain.on("set-volume", async (_event: IpcMainEvent, vol: number) => {
    await setVolume(vol);
  });
  ipcMain.handle("get-recordings", async () => {
    ...
    try {
      ...
      const err = error as Error;
      logger.error("SYSTEM", `Failed to get recordings: ${err.message}`);
      return [];
    }
  });
  ...
  ipcMain.handle(
    "delete-recording",
    async (_event: IpcMainInvokeEvent, dirName: string) => {
      if (
        !dirName ||
        dirName.includes("..") ||
        dirName.includes("/") ||
      ...
      try {
        ...
        const err = error as Error;
        logger.error("SYSTEM", `Failed to delete recording: ${err.message}`);
        return false;
      }
    },
  );
  ipcMain.handle(
    "save-recording",
    async (_event: IpcMainInvokeEvent, data: SaveRecordingPayload) => {
      try {
        ...
        if (videoBuffer) {
          writePromises.push(
            fs.promises.writeFile(videoFile, Buffer.from(videoBuffer as any)),
          );
        }

        if (micBuffer) {
          writePromises.push(
            fs.promises.writeFile(micFile, Buffer.from(micBuffer as any)),
          );
        }
        if (musicBuffer) {
          writePromises.push(
            fs.promises.writeFile(musicFile, Buffer.from(musicBuffer as any)),
          );
        }
        ...
        logger.error(
          "SYSTEM",
          `Failed to save recording session: ${err.message}`,
        );
  ...
  ipcMain.handle("select-soundfont-file", async (event: IpcMainInvokeEvent) => {
    ...
    const result = await dialog.showOpenDialog(win, {
      ...
      filters: [
        {
          name: "Soundfont Files",
          extensions: ["sf2", "sf3", "sfz", "dls"],
        },
  ...
  ipcMain.handle(
    "libmgr-export-songbook",
    async (event: IpcMainInvokeEvent, payload: SongbookExportPayload) => {
      const win = BrowserWindow.fromWebContents(event.sender);
      if (!win) return { success: false, reason: "Window not found." };
      ...
      const view = mainWin?.contentView.children[0] as
        WebContentsView | undefined;
      if (mainWin && view) {
        view.webContents.send("request-songbook-data", {
          folderPath,
          filePath,
          libraryTitle,
        });
  ...
  ipcMain.on(
    ...
    (_event: IpcMainEvent, data: SongbookProgressPayload) => {
  ...
  ipcMain.on(
    "receive-songbook-data",
    (_event: IpcMainEvent, payload: SongbookReceivePayload) => {
      isSongbookBuildActive = true;
      ...
      let fontRajdhani: Buffer;
      let fontRajdhaniBold: Buffer;
      let fontRadioCanada: Buffer;
      let fontJP: Buffer | undefined;
      let fontKR: Buffer | undefined;
      let fontSC: Buffer | undefined;
      ...
      try {
        ...
        const err = fontErr as Error;
        logger.error("SYSTEM", `PDF Font Error: ${err.message}`);
        BrowserWindow.getAllWindows().forEach((w) =>
          w.webContents.send("songbook-export-complete", {
            success: false,
            reason: err.message,
      ...
      const getFontForText = (text: string, defaultFont: string): string => {
      ...
      try {
        ...
        logger.error("SYSTEM", `PDF Generation crashed: ${error.message}`);
        BrowserWindow.getAllWindows().forEach((w) =>
          w.webContents.send("songbook-export-complete", {
            success: false,
  ...
  ipcMain.on("songbook-export-complete", () => {
  ...
  ipcMain.handle(
    ...
    async (
      event: IpcMainInvokeEvent,
      {
        ...
        serverPort,
        libraryPath,
      }: { serverIp: string; serverPort: number; libraryPath: string },
    ) => {
      try {
        ...
        if (fs.existsSync(hashCachePath)) {
          try {
            hashCache = JSON.parse(
              fs.readFileSync(hashCachePath, "utf8"),
            ) as HashCache;
          } catch {
            logger.warn(
              "SYNC",
        ...
        const remoteFiles = (await res.json()) as RemoteSyncFile[];

        const toDownload: RemoteSyncFile[] = [];
            ...
            const req = http
              .get(fileUrl, { timeout: 15000 }, (response) => {
                if (response.statusCode !== 200) {
                  return reject(
                    new Error(
                      `Failed to get '${safeName}' (Status: ${response.statusCode ?? "unknown"})`,
                    ),
                  );
                }
  ...
  ipcMain.on("open-library-manager", () => {
    ...
    void libraryManagerWin.loadURL(
  ...
  ipcMain.handle("libmgr-get-config", () => {
    const knownPaths = (Config.getItem("knownLibraries") as string[]) || [];
    const libraries: KnownLibraryItem[] = [];
    ...
    return {
      active: Config.getItem("libraryPath") as string | null,
      libraries: libraries,
    };
  });
  ...
  ipcMain.handle("libmgr-scan-drives", async () => {
    try {
      const disks = await si.fsSize();
      const mountPoints = [...new Set(disks.map((d) => d.mount))];
      const known = (Config.getItem("knownLibraries") as string[]) || [];
      let foundCount = 0;
  ...
  ipcMain.on(
    ...
    (_event: IpcMainEvent, targetPath: string) => {
      if (fs.existsSync(targetPath)) void shell.openPath(targetPath);
    },
  );

  ipcMain.handle(
    "libmgr-get-library-contents",
  ...
  ipcMain.handle(
    "libmgr-select-bgv-files",
    async (
      event: IpcMainInvokeEvent,
      folderPath: string,
    ): Promise<{ relativePaths: string[]; outsideCount: number }> => {
      const win = BrowserWindow.fromWebContents(event.sender);
      if (!win) return { relativePaths: [], outsideCount: 0 };
      const result = await dialog.showOpenDialog(win, {
  ...
  ipcMain.handle(
    "libmgr-update-manifest",
    (
      _event: IpcMainInvokeEvent,
      payload: { folderPath: string; manifestData: LibraryManifest },
    ) => {
      try {
        ...
        logger.error(
          "SYSTEM",
          `Failed to save manifest at ${payload?.folderPath}: ${err.message}`,
        );
        return { success: false, error: err.message };
      }
    },
  ...
  ipcMain.handle(
    "libmgr-remove-library",
    async (_event: IpcMainInvokeEvent, folderPath: string) => {
      let known = (Config.getItem("knownLibraries") as string[]) || [];
      known = known.filter((p: string) => p !== folderPath);
      Config.setItem("knownLibraries", known);
  ...
  ipcMain.handle(
    "save-songbook-cache",
    async (_event: IpcMainInvokeEvent, payload: SaveCachePayload) => {
      try {
        ...
        const err = e as Error;
        logger.error("SYSTEM", `Failed to save embedded cache: ${err.message}`);
        return { success: false, error: err.message };
      }
    },
  );

  ipcMain.on("setRPC", (event: IpcMainEvent, arg: RPCSetPayload) => {
    ...
    void discordClient.user?.setActivity(activityPayload);

    const win = BrowserWindow.fromWebContents(event.sender);
    if (win) win.webContents.send("update-now-playing", arg);
  });

  const knownRemotes: Record<string, RemoteInfo> = {};

  cloudSocket.on("remote-connected", ({ identity }: { identity: string }) => {
  ...
  cloudSocket.on(
    "remote-command",
    (payload: { identity: string; data: unknown }) => {
      logger.debug(
        "LINK",
  ...
  io.on("connection", (socket: Socket) => {
    ...
    if (clientType === "app") {
      ...
      void socket.join("karaoke-app");
      socket.emit("remotes", knownRemotes);

      socket.on(
        "sendData",
        (msg: { identity?: string; data: unknown } | undefined) => {
          if (!msg) return;
      ...
      socket.on("broadcastData", (msg: unknown) => {
    ...
    if (clientType === "remote") {
      ...
      socket.on("remote-command", (data: unknown) => {
        logger.debug(
          "LINK",
  ...
  serverHttp.on("error", (err: NodeJS.ErrnoException) => {
  ...
  serverHttp.on("listening", () => {
    ...
    PORT = typeof address === "object" && address ? address.port : tryPort;
    ...
    instance.find({ type: "enmoku" }, (service: Service) => {
