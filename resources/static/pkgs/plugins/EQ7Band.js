...
// src/pkgs/plugins/EQ7Band.js
var BANDS = [60, 150, 400, 1e3, 2400, 6e3, 15e3];
var EQ7BandPlugin = class extends BasePlugin {
  ...
  renderGUI(wrapper, Html) {
    ...
    this.bandElements = [];
    const eqContainer = new Html("div").styleJs({
      display: "flex",
      gap: "1rem",
    ...
    BANDS.forEach((freq, idx) => {
      const bandCol = new Html("div").styleJs({
        display: "flex",
      ...
      const faderThumb = new Html("div").styleJs({
        ...
        // Removed 'bottom' transition for instant 1:1 mouse tracking
      }).appendTo(trackContainer);
      ...
      const freqLabel = new Html("div").styleJs({
        fontFamily: "'Rajdhani', sans-serif",
        fontWeight: "600",
        fontSize: "1.1rem",
      ...
      }).text(`${freq >= 1e3 ? freq / 1e3 + "k" : freq}Hz`).appendTo(bandCol);
...
export {
  EQ7BandPlugin as default
};
//# sourceMappingURL=EQ7Band.js.map
