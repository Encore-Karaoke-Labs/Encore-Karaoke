...
// src/pkgs/plugins/Compressor.js
var CONTROLS = [
  {
    key: "inputGain",
    label: "IN GAIN",
...
var CompressorPlugin = class extends BasePlugin {
  constructor(audioContext) {
    ...
    this.parameters = {
      ...
      release: { min: 10, max: 1e3, value: 250 },
      outputGain: { min: -24, max: 24, value: 0 }
    };
    this.updateAllParameters();
    this.input.connect(this.inputGain).connect(this.compressorNode).connect(this.outputGain).connect(this.output);
  ...
  updateAllParameters() {
    ...
    this.compressorNode.attack.value = this.parameters.attack.value / 1e3;
    this.compressorNode.release.value = this.parameters.release.value / 1e3;
  ...
  setParameter(key, value) {
    if (this.parameters[key] === void 0) return;
  ...
  renderGUI(wrapper, Html) {
    ...
    const compContainer = new Html("div").styleJs({
      display: "flex",
      gap: "0.75rem",
