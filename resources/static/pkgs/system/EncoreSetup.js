...
// src/pkgs/system/EncoreSetup.js
var EncoreSetupController = class {
  /**
  ...
  async init() {
    ...
    this.updateServers = await window.desktopIntegration.ipc.invoke("get-update-servers");
    ...
    this.currentManifest = activeLib ? activeLib.manifest : { title: "Unknown", description: "No metadata available." };
    ...
    this.container = new Html("div").classOn("setup-container").appendTo(this.wrapper);
  ...
  async verifyPin(input) {
    ...
    const actualPort = await networkingUtility_default.getPort();
  ...
  async createPinHash(input) {
    let actualPort = await networkingUtility_default.getPort();
    try {
      const res = await fetch(
  ...
  buildSettingsMap() {
    this.DASHBOARD_TILES = [
      { id: "library", label: "Library & Storage", icon: "\u{1F4C1}" },
      { id: "sync", label: "Network Sync", icon: "\u{1F310}" },
      { id: "audio", label: "Sound Settings", icon: "\u{1F50A}" },
      { id: "mic", label: "Microphone Settings", icon: "\u{1F3A4}" },
      { id: "video", label: "Video Settings", icon: "\u{1F4FA}" },
      { id: "security", label: "User Security", icon: "\u{1F512}" },
      { id: "reboot", label: "System Reboot", icon: "\u{1F504}" }
    ];
    ...
    const serverOptions = this.updateServers && this.updateServers.length > 0 ? this.updateServers.map((s) => ({ label: s.host, value: s.id })) : [{ label: "No servers found", value: "none" }];
    if (!this.state.selectedServerId || !serverOptions.find((o) => o.value === this.state.selectedServerId)) {
          ...
          },
          {
            ...
            action: () => {
              this.state.dialog = {
                ...
                content: this.currentManifest?.description || "No description provided by this library."
              };
              this.renderView();
            }
          },
        ...
        items: [
          ...
          },
          {
            ...
            action: async () => {
              ...
              this.updateServers = await window.desktopIntegration.ipc.invoke(
                "get-update-servers"
              );
              this.buildSettingsMap();
              this.renderView();
        ...
        items: [
          {
            ...
            get: () => this.config.audioConfig?.mix?.instrumental?.outputDevice || "default",
            set: (v) => {
          ...
          },
          {
            ...
            max: 1e3,
            step: 10,
            get: () => Math.round((this.config.audioConfig?.bufferSize ?? 0.1) * 1e3),
            set: (v) => {
              const val = v / 1e3;
              this.config.audioConfig ??= {};
        ...
        items: [
          {
            ...
            type: "select",
            options: micOptions,
            get: () => this.config.audioConfig?.mix?.scoring?.inputDevice || "default",
            set: (v) => {
              this.config.audioConfig ??= {};
              this.config.audioConfig.mix ??= {};
          ...
          },
          {
            ...
            max: 1e3,
            step: 10,
            get: () => Math.round((this.config.audioConfig?.micLatency ?? 0) * 1e3),
            set: (v) => {
              const val = v / 1e3;
              this.config.audioConfig ??= {};
  ...
  handleKeyDown(e) {
    if (this.state.manualCalib && this.state.manualCalib.active) {
      ...
      if (this.state.manualCalib.phase === "input") {
        if (e.key >= "0" && e.key <= "9") {
          ...
          this.state.manualCalib.songInput = this.state.manualCalib.songInput.slice(0, -1);
          this.renderView();
        } else if (e.key === "Enter") {
        ...
        if (e.key === "ArrowLeft" || e.key === "ArrowRight") {
          ...
          this.state.manualCalib.offset = Math.max(
            0,
            Math.min(1e3, this.state.manualCalib.offset + 10 * dir)
          );
    ...
    if (this.state.previewingVideo) {
      if (e.key === "ArrowLeft" || e.key === "ArrowRight") {
        ...
        const newOffset = Math.max(
          -1e3,
          Math.min(1e3, currentOffset + 10 * dir)
        );
        this.config.videoConfig ??= {};
    ...
    if (e.key === "Escape") {
      if (this.state.view === "submenu" || this.state.view === "pin_change") {
        ...
        this.renderView();
      } else if (this.state.view === "dashboard" || this.state.view === "auth") {
    ...
    if (this.state.view === "dashboard") {
      ...
      if (e.key === "ArrowLeft")
        this.state.dashboardIndex = (this.state.dashboardIndex - 1 + total) % total;
      if (e.key === "ArrowDown")
    ...
    if (this.state.view === "submenu") {
      ...
      if (e.key === "ArrowDown") {
        ...
        if (currentItem.type === "range") {
          ...
          const nextIndex = (currentIndex + dir + currentItem.options.length) % currentItem.options.length;
          currentItem.set(currentItem.options[nextIndex].value);
        }
      } else if (e.key === "Enter" && (currentItem.type === "action" || currentItem.type === "info-action")) {
        currentItem.action();
      }
      this.renderView();
    }
  ...
  renderBuildOverlay(container) {
    const overlay = new Html("div").classOn("setup-manual-calib-overlay").appendTo(container);
    new Html("h2").text("BUILDING SONG LIST").appendTo(overlay);
    new Html("p").text("Scanning library files and extracting metadata...").appendTo(overlay);
    const progressContainer = new Html("div").styleJs({ width: "80%", maxWidth: "600px", marginTop: "2rem" }).appendTo(overlay);
    const barBg = new Html("div").classOn("setup-slider-bar").styleJs({ height: "20px" }).appendTo(progressContainer);
    this.buildProgressBar = new Html("div").classOn("setup-slider-fill").styleJs({ width: `${this.state.buildProgress.percentage}%` }).appendTo(barBg);
    this.buildProgressText = new Html("p").styleJs({
      marginTop: "1rem",
      color: "#89cff0",
      fontSize: "1.2rem",
      fontWeight: "bold"
  ...
  async startNetworkSync() {
    if (this.state.selectedServerId === "none" || !this.updateServers || this.updateServers.length === 0) {
  ...
  renderSyncOverlay(container) {
    ...
    new Html("p").text("Checking for differences and downloading missing files...").appendTo(overlay);
    const progressContainer = new Html("div").styleJs({
      width: "90%",
      maxWidth: "700px",
      marginTop: "2rem",
      display: "flex",
    ...
    const overallLabel = new Html("div").styleJs({
      display: "flex",
    ...
    this.syncOverallText = new Html("span").text("0 / 0").appendTo(overallLabel);
    const barBgOverall = new Html("div").classOn("setup-slider-bar").styleJs({ height: "15px", background: "rgba(0,0,0,0.6)" }).appendTo(overallWrap);
    this.syncProgressBarOverall = new Html("div").classOn("setup-slider-fill").styleJs({ width: "0%", backgroundColor: "#ffd700" }).appendTo(barBgOverall);
    const fileWrap = new Html("div").appendTo(progressContainer);
    const fileLabel = new Html("div").styleJs({
      display: "flex",
      justifyContent: "space-between",
      color: "#89cff0",
      fontWeight: "bold",
    ...
    this.syncProgressText = new Html("span").styleJs({
      whiteSpace: "nowrap",
      overflow: "hidden",
      textOverflow: "ellipsis",
      maxWidth: "70%"
    }).text("Connecting...").appendTo(fileLabel);
    this.syncFileBytesText = new Html("span").text("0.00 MB / 0.00 MB").appendTo(fileLabel);
    const barBgFile = new Html("div").classOn("setup-slider-bar").styleJs({ height: "15px", background: "rgba(0,0,0,0.6)" }).appendTo(fileWrap);
    this.syncProgressBarFile = new Html("div").classOn("setup-slider-fill").styleJs({ width: "0%", backgroundColor: "#89cff0" }).appendTo(barBgFile);
  }
  /**
  ...
  startManualCalibration() {
    this.state.manualCalib = {
      ...
      phase: "input",
      songInput: "",
      offset: Math.round((this.config.audioConfig?.micLatency ?? 0) * 1e3),
  ...
  async startCalibrationRecording(song) {
    ...
    this.boundCalibTimeUpdate = (e) => {
      ...
      if (this.state.manualCalib.lrcData.length > 0) {
        ...
        if (activeIdx !== currentLineIdx) {
          ...
          const activeLine = activeIdx >= 0 ? this.state.manualCalib.lrcData[activeIdx].text : "Start singing!";
          const nextLine = activeIdx >= 0 && activeIdx + 1 < this.state.manualCalib.lrcData.length ? this.state.manualCalib.lrcData[activeIdx + 1].text : "";
    ...
    if (this.calibLyricLine2) {
      if (this.state.manualCalib.isMidi && this.state.manualCalib.midiLines.length > 0) {
        this.calibLyricLine2.text(this.state.manualCalib.midiLines[0].text);
      } else if (!this.state.manualCalib.isMidi && this.state.manualCalib.lrcData.length > 0) {
  ...
  stopCalibrationRecording() {
    ...
    Promise.all([p1, p2]).then(async ([micArray, musicArray]) => {
      const ctx = this.state.manualCalib.audioContext;
      this.state.manualCalib.micBuffer = await ctx.decodeAudioData(micArray);
      this.state.manualCalib.trackBuffer = await ctx.decodeAudioData(musicArray);
  ...
  startCalibrationPlayback() {
    ...
    this.state.manualCalib.trackSource.buffer = this.state.manualCalib.trackBuffer;
    ...
    this.state.manualCalib.trackDelayNode = ctx.createDelay(2);
    this.updateCalibrationDelay();
    this.state.manualCalib.trackSource.connect(this.state.manualCalib.trackDelayNode).connect(ctx.destination);
  ...
  updateCalibrationDelay() {
    ...
    const offsetSeconds = Math.max(0, this.state.manualCalib.offset / 1e3);
  ...
  renderWaveforms() {
    ...
    const drawBuffer = (buffer, color, yOffset, heightScale, timeOffsetMs = 0) => {
      const data = buffer.getChannelData(0);
      const sampleRate = buffer.sampleRate;
      const offsetSeconds = timeOffsetMs / 1e3;
      ctx.beginPath();
  ...
  saveManualCalibration() {
    const val = this.state.manualCalib.offset / 1e3;
    this.config.audioConfig ??= {};
    this.config.audioConfig.micLatency = val;
    window.config.setItem("audioConfig.micLatency", val);
    this.Forte.setLatency(val);
  ...
  async executeAction(id) {
    if (id === "reboot") {
      const fadeOverlay = new Html("div").styleJs({
        position: "fixed",
        top: 0,
        left: 0,
        width: "100vw",
  ...
  syncVideoLoop() {
    ...
    const pbState = this.Forte.getPlaybackState();
    if (pbState.status === "playing" && this.previewVideoEl && this.previewVideoEl.elm.readyState >= 2) {
      const vid = this.previewVideoEl.elm;
      const offsetSec = (this.config.videoConfig?.syncOffset || 0) / 1e3;
      const target = pbState.currentTime + offsetSec;
      const drift = (target - vid.currentTime) * 1e3;
   ...
   */
  showToast(msg, type) {
    const toast = new Html("div").classOn("setup-toast", type).text(msg).appendTo(this.wrapper);
  ...
  renderView() {
    ...
    const header = new Html("div").classOn("setup-header").appendTo(this.container);
    ...
    const footer = new Html("div").classOn("setup-footer").appendTo(this.container);
  ...
  renderManualCalibrationOverlay(container) {
    ...
    if (this.state.manualCalib.phase === "input") {
      ...
      const displayCode = this.state.manualCalib.songInput.padStart(5, "0");
      new Html("div").classOn("calib-value-display").text(displayCode).appendTo(overlay);
      const song = this.songList.find((s) => s.code === displayCode);
      if (song) {
        new Html("p").styleJs({ color: "#89cff0", fontWeight: "bold", fontSize: "1.5rem" }).text(`${song.title} - ${song.artist}`).appendTo(overlay);
      } else if (this.state.manualCalib.songInput.length > 0) {
        new Html("p").styleJs({ color: "#ff5555", fontWeight: "bold", fontSize: "1.5rem" }).text("Song not found in library.").appendTo(overlay);
      }
      new Html("p").styleJs({ marginTop: "2rem", opacity: "0.6" }).text("Use Number Keys to type | ENTER to Start | ESC to Cancel").appendTo(overlay);
    } else if (this.state.manualCalib.phase === "recording") {
      new Html("div").classOn("calib-status-badge").text("RECORDING... SING ALONG!").appendTo(overlay);
      const lrcCont = new Html("div").classOn("calib-lyrics-container").appendTo(overlay);
      this.calibLyricLine1 = new Html("div").classOn("calib-lyric-line", "active").text("Loading track...").appendTo(lrcCont);
      this.calibLyricLine2 = new Html("div").classOn("calib-lyric-line", "next").text("").appendTo(lrcCont);
      new Html("p").styleJs({ marginTop: "1rem" }).text("Press ENTER when you are finished singing to begin adjusting.").appendTo(overlay);
      ...
      new Html("br").appendTo(overlay);
      const controls = new Html("div").classOn("calib-controls").appendTo(overlay);
      const layout = new Html("div").classOn("calib-waveform-layout").appendTo(controls);
      const labels = new Html("div").classOn("calib-waveform-labels-side").appendTo(layout);
      new Html("span").classOn("calib-label-music").text("MUSIC").appendTo(labels);
      new Html("span").classOn("calib-label-mic").text("MIC").appendTo(labels);
      this.calibCanvas = new Html("canvas").classOn("calib-waveform-canvas").attr({ width: 800, height: 200 }).appendTo(layout);
      const sliderBox = new Html("div").classOn("calib-slider-container").appendTo(controls);
      const offset = this.state.manualCalib.offset;
      this.calibOffsetDisplay = new Html("div").classOn("calib-value-display").text(`${offset > 0 ? "+" : ""}${offset} ms`).appendTo(sliderBox);
      const btns = new Html("div").classOn("calib-buttons").appendTo(controls);
      new Html("button").classOn("box", "positive").text("Save & Exit (ENTER)").on("click", () => this.saveManualCalibration()).appendTo(btns);
      new Html("button").classOn("box", "negative").text("Discard (ESC)").on("click", () => this.exitManualCalibration()).appendTo(btns);
    }
  }
  /**
  ...
  renderVideoPreviewOverlay(container) {
    const overlay = new Html("div").classOn("setup-video-preview-overlay").appendTo(container);
    this.previewVideoEl = new Html("video").attr({ muted: true }).classOn("setup-preview-video").appendTo(overlay);
    const hud = new Html("div").classOn("setup-preview-hud").appendTo(overlay);
    new Html("h2").text("VIDEO SYNC CALIBRATION").appendTo(hud);
    const currentOffset = this.config.videoConfig?.syncOffset || 0;
    this.offsetDisplay = new Html("div").classOn("setup-preview-offset").text(`OFFSET: ${currentOffset > 0 ? "+" : ""}${currentOffset} ms`).appendTo(hud);
  ...
  renderDialog(container) {
    const overlay = new Html("div").classOn("setup-dialog-overlay").appendTo(container);
    const box = new Html("div").classOn("setup-dialog-box").appendTo(overlay);
    new Html("h2").text(this.state.dialog.title).appendTo(box);
    new Html("div").classOn("setup-dialog-content").text(this.state.dialog.content).appendTo(box);
    new Html("p").classOn("setup-dialog-hint").text("Press ENTER or ESC to close").appendTo(box);
  }
  /**
  ...
  renderDashboard(container) {
    const scrollWrapper = new Html("div").classOn("dashboard-scroll-wrapper", "submenu-list").styleJs({
      width: "100%",
      maxHeight: "100%",
      overflowY: "auto",
      paddingTop: "0.5rem"
    }).appendTo(container);
    const grid = new Html("div").classOn("setup-grid").styleJs({
      margin: "0 auto",
      paddingBottom: "2rem"
    ...
    this.DASHBOARD_TILES.forEach((tile, idx) => {
      ...
      new Html("div").classOn("setup-tile-icon").text(tile.icon).appendTo(tileEl);
      new Html("div").classOn("setup-tile-label").text(tile.label).appendTo(tileEl);
    });
  ...
  renderSubmenu(container) {
    ...
    new Html("h2").classOn("submenu-title").text(menuData.title).appendTo(panel);
...
var EncoreSetup_default = pkg;
export {
  EncoreSetup_default as default
};
//# sourceMappingURL=EncoreSetup.js.map
