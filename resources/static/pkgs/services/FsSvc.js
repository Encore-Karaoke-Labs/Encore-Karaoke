...
var pkg = {
  ...
  data: {
    ...
    findEncoreLibraries: async () => {
      ...
      const allPaths = [.../* @__PURE__ */ new Set([...knownPaths, ...legacyPaths])];
    ...
    buildSongList: async (libraryPath) => {
      ...
      const audioExtensions = /* @__PURE__ */ new Set(["wav", "mp3", "m4a"]);
      const videoExtensions = /* @__PURE__ */ new Set(["mp4", "mkv", "webm", "avi"]);
      const validExts = /* @__PURE__ */ new Set([
        ...audioExtensions,
        ...videoExtensions,
        "mid",
        "kar",
      ...
      const currentSignature = [...files].filter((f) => validExts.has(f.name.split(".").pop().toLowerCase())).sort((a, b) => a.name.localeCompare(b.name)).map((f) => `${f.name}:${f.modified}`).join("|");
      const cachedSignature = await import_localforage.default.getItem(signatureKey);
      const cachedList = await import_localforage.default.getItem(cacheKey) || [];
      const cachedNewSongs = await import_localforage.default.getItem(newSongsKey) || [];
      ...
      const processableFiles = files.filter(
        (file) => file.type === "file" && (audioExtensions.has(file.name.split(".").pop().toLowerCase()) || file.name.endsWith(".mid") || file.name.endsWith(".kar"))
      );
      let processed = 0;
      ...
      for (const file of processableFiles) {
        ...
        if (audioExtensions.has(extension) && allFilenames.has(`${basename}.lrc`)) {
          songData = {
      ...
      await import_localforage.default.setItem(cacheKey, newSongList);
      await import_localforage.default.setItem(signatureKey, currentSignature);
      await import_localforage.default.setItem(newSongsKey, state.newSongs);
      state.isBuilding = false;
      dispatchSongListReady();
      return true;
    },
...
var FsSvc_default = pkg;
export {
  FsSvc_default as default
};
//# sourceMappingURL=FsSvc.js.map
